...
CTime CCheckManage::GetTimeForStr(CString timestr)
{
	...
	if (timestr.GetLength() < 8)
		timestr = "0"+timestr;
	hour = atoi(timestr.Left(2));
	minute = atoi(timestr.Mid(3,2));
	second = atoi(timestr.Right(2));
	CTime result(2000,1,1,hour,minute,second);
	return result;
}
...
{
	int year,mounth,day;
	year = atoi(datestr.Left(4));
	mounth = atoi(datestr.Mid(5,2));
	day = abs(atoi(datestr.Right(2)));
	CTime result(year,mounth,day,8,0,0);
	return result;
}
...
void CCheckManage::OnAppend() 
{
	...
	if (checkedit.DoModal() == IDOK)
	{
		...
		for(int i=0;i<dataset.GetRecordCount();i++)
		{
			...
			if(str.Compare(checkedit.m_name) == 0)
			{
				MessageBox("该员工已存在！");
				return;
			}
...
void CCheckManage::OnEdit() 
{
    ...
    checkedit.m_datecheck = GetTimeForStr((char *)(_bstr_t)dataset.GetFields()->Item["checkdate"]->Value);
	if (checkedit.DoModal() == IDOK)
	{
		CString time;
		dataset.SetFieldValue("name",(_bstr_t)checkedit.m_name);

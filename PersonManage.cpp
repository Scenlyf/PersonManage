...
void CPersonManage::OnAppend() 
{
	...
	if (personedit.DoModal() == IDOK)
	{
		...
		{
			CString id = m_list.GetItemText(i,0);
			if(id.Compare(personedit.m_id) == 0)//员工编码相同，用户已经存在，不用姓名作比较（同名）
			{
				MessageBox("该员工已存在！");
				return;
			}

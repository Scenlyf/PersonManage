...
{
	CString Str;
	Str.Format("Provider=SQLOLEDB.1;Password=970301;Persist Security Info=True;User ID=sa;Initial Catalog=%s;Data Source=%s",DBName,IP);//这地方要改动
	//Str.Format("Provider=SQLOLEDB.1; Persist Security Info=False;Initial Catalog=%s;Data Source=%s",DBName,IP);
	return Str;
}
...
int CADODataSet::GetRecordCount()//返回记录的个数
{
	if (IsOpen())
